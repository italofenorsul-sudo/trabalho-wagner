...
router.get('/', exigirLogin, (req, res) => {
  ...
  // Ponto-chave de integridade: o valor total é sempre RECALCULADO no
  // servidor a partir do banco (SUM), nunca aceito como número pronto
  // vindo do front-end. Ninguém consegue "editar" esse total manipulando
  // a requisição — o servidor não confia em nada que não tenha calculado
  // ele mesmo a partir dos dados que ele controla.
  const valorTotalEstimado = db
    .prepare(`SELECT COALESCE(SUM(valor_estimado), 0) AS total FROM licitacoes WHERE 1=1${visivelSql}`)
    .get().total;
